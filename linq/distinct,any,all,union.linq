...
//Show Genres that have tracks which are not on any playlist (which are not popular)
...
//	 if all of the items match the specific condition
//returns no data just a true or false
...
//In either case you are comparing on collection to a second collection

//obtain a distinct list of all playlist tracks for Roberto Almeida
//	 username of AlmediaR
...
//Create a List of tracks that Roberto has but and Michelle doesn't
...
//Create a List of tracks that Michelle has but and Roberto doesn't
//using .Any()
...
//to concatenate two the results from multiple queries
...
//Average() is returned as a double
//Count() is an integer
//Sum() is a decimal
